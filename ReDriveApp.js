...
function createFile(a1, a2, a3, folder_id) {
  ...
  if (signature === CREATE_FILE_SIG_BLOB) {
    newFile = createFileFromBlob_(a1, folder_id);
  } else if (signature === CREATE_FILE_SIG_NC ) {
    newFile = createFileFromContentAndMimetype_(a1, a2, 'text/plain', folder_id);
  } else if (signature === CREATE_FILE_SIG_NCM ) {
    newFile = createFileFromContentAndMimetype_(a1, a2, a3, folder_id);
  }

  return newFile;  
}

function createFileFromBlob_(blob, folder_id) {
...
function createFileFromContentAndMimetype_(name, content, mimeType, folder_id) {
  if (getDriveApiVersion_() === 3) {
    throw new Error('createFile() not yet supported by ReDriveApp for Drive API v3')
  } else {
